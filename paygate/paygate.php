...
HTML;
        ...
        foreach ($this->paygatePayMethods as $key => $paygatePayMethod) {
            ...
            if (Configuration::get($k) != '') {
                $payOptionsHtml .= <<<HTML
<p><input type="radio" value="$key" name="paygatePayMethodRadio">
                    ...
                    array(
                        ...
                        'values' => array(
                            'query' => array(
                                ...
                                array(
                                    'id'   => 'snapscan',
                                    'name' => 'Snapscan<img src="../modules/paygate/assets/images/snapscan.svg" alt="Sanpscan" style="height: 15px; margin-left: 10px;">',
                                    'val'  => 'snapscan',
                                ),
                                array(
                                    'id'   => 'mobicred',
    ...
    public function createOrderCartRulesViaPaygate(
        ...
        foreach ( $cartRulesData as $cartRuleData ) {
            ...
            if ( count(
				...
				$values = $this->createNewVoucher($cartRule,$order,$values,$total_reduction_value_ti,$total_reduction_value_tex);
            }
